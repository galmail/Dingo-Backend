class UserNotifier < ActionMailer::Base
  ...
  def send_signup_email(user)
    @user = user
    mail(:from => self.dingo_email, :to => @user.email,:subject => 'Thanks for signing up to Dingo' )
  end
  ...
  def confirm_reported_user_to_notifier(notifier_user,reported_user)
    @reported_user = reported_user
    @notifier_user = notifier_user
    mail(:from => self.dingo_email,:to => notifier_user.email,:subject => 'You have reported a user')
  end
